...
<c:set var="title"><spring:message code="page.title.exercise.edit" /></c:set>
...
<section>
    <div class="row justify-content-center mb-5 border border-2 text-wrapper">
        <div class="col container-fix-width">
            <h1 class="fs-2 mb-0 py-3 ps-3"><spring:message code="text.exercise.edit" /></h1>
        </div>
    </div>
    <div class="row justify-content-center">
        <div class="col container-fix-width">
            <form:form modelAttribute="exerciseDTO" action="/exercise/edit" method="post"
                       class="card border shadow p-3" >
                <form:hidden path="id" />
                <form:hidden path="trainingPlanId" />
                <form:hidden path="exerciseType" />
                <div class="mb-3">
                ...
                <div class="mb-3">
                    ...
                    <form:input path="sets" type="number" min="1" max="10" cssClass="form-control" maxlength="10" />
                ...
                <c:if test="${exerciseDTO.exerciseType == ExerciseType.REPETITIVE}">
                    <div class="mb-3">
                        <form:label path="repetitions" cssClass="form-label"><spring:message code="label.exercise.repetitions" /></form:label>
                        <form:input path="repetitions" type="number" min="1" max="30" cssClass="form-control" />
                        <form:errors cssClass="text-danger" path="repetitions"/>
                    </div>
                </c:if>
                <c:if test="${exerciseDTO.exerciseType == ExerciseType.INTERVAL}">
                    <div class="mb-3">
                        <form:label path="length" cssClass="form-label"><spring:message code="label.exercise.length" /></form:label>
                        <div class="input-group">
                            <form:input path="length" type="number" min="1" max="180" cssClass="form-control border-end border-2" />
                            <form:select path="lengthTimeUnit" cssClass="form-select">
                                <form:options items="${timeUnits}" />
                            </form:select>
                        </div>
